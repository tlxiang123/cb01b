...
typedef struct
{
    unsigned int command_size;              /**< The size of the current init command stored in the DFU settings. */
    unsigned int command_offset;            /**< The offset of the currently received init command data. The offset will increase as the init command is received. */
    unsigned int command_crc;               /**< The calculated CRC of the init command (calculated after the transfer is completed). */
    unsigned int data_object_size;          /**< The size of the last object created. Note that this size is not the size of the whole firmware image.*/
    struct
    {
        unsigned int firmware_file_version;
        unsigned int firmware_file_length;
        unsigned int firmware_file_crc;
        unsigned char  firmware_file_md5[16];        
    };
    union
    {
        struct
        {
            unsigned int firmware_image_crc;        /**< CRC value of the current firmware (continuously calculated as data is received). */
            unsigned int firmware_image_crc_last;   /**< The CRC of the last executed object. */
            unsigned int firmware_image_offset;     /**< The offset of the current firmware image being transferred. Note that this offset is the offset in the entire firmware image and not only the current object. */
            unsigned int firmware_image_offset_last;/**< The offset of the last executed object from the start of the firmware image. */
        };
        struct
        {
            unsigned int update_start_address;      /**< Value indicating the start address of the new firmware (before copy). It's always used, but it's most important for an SD/SD+BL update where the SD changes size or if the DFU process had a power loss when updating a SD with changed size. */
        };
    };
} dfu_progress_t;
...
typedef struct
{
    unsigned int                image_size;         /**< Size of the image in the bank. */
    unsigned int                image_crc;          /**< CRC of the image. If set to 0, the CRC is ignored. */
    unsigned int                bank_code;          /**< Identifier code for the bank. */
} dfu_bank_t;

typedef struct
{
    unsigned int            crc;                /**< CRC for the stored DFU settings, not including the CRC itself. If 0xFFFFFFF, the CRC has never been calculated. */
    unsigned int            settings_version;   /**< Version of the current DFU settings struct layout. */
    unsigned int            app_version;        /**< Version of the last stored application. */
    unsigned int            bootloader_version; /**< Version of the last stored bootloader. */

    unsigned int            bank_layout;        /**< Bank layout: single bank or dual bank. This value can change. */
    unsigned int            bank_current;       /**< The bank that is currently used. */

    dfu_bank_t      bank_0;             /**< Bank 0. */
    dfu_bank_t      bank_1;             /**< Bank 1. */

    unsigned int            write_offset;       /**< Write offset for the current operation. */
    unsigned int            sd_size;            /**< Size of the SoftDevice. */

    dfu_progress_t      progress;           /**< Current DFU progress. */

    unsigned int            enter_buttonless_dfu;
    unsigned char             init_command[256];  /**< Buffer for storing the init command. */
} dfu_settings_t;
