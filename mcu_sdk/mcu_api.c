...
*****************************************************************************/
void dbg_uart_rx_buff(void) {
	uint16_t total_len = 0;
	total_len = bt_uart_rx_buf[LENGTH_HIGH] * 0x100;
    total_len += bt_uart_rx_buf[LENGTH_LOW] + PROTOCOL_HEAD;
	DBG_UART2("\r\nMcu rx data:  ");
	  for (int i =0; i < total_len; i++) {
		DBG_UART2("%02X ", bt_uart_rx_buf[i]);
	  }
	DBG_UART2("\r\n");
}
...
void bt_uart_service(void)
{
  ...
  if((rx_in < sizeof(bt_uart_rx_buf)) && get_queue_total_data() > 0)
  {
    ...
    if(mcu_common_uart_data_unpack(Queue_Read_Byte()))
    {
      ...
      {
          if (bt_uart_rx_buf[3] != 00 && bt_uart_rx_buf[3] != 07 && bt_uart_rx_buf[3] != 0xE1) {
            dbg_uart_rx_buff();
          }
          data_handle(0);
