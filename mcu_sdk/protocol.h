...
******************************************************************************/
#define PRODUCT_KEY "lakcajdb"    //开发平台创建产品后生成的16位字符产品唯一标识
...
#define BT_UART_RECV_BUF_LMT          64              //According to the size of the user's DP data, it must be greater than 32
#else
...
//单次水量(只上报)
//备注:
#define DPID_WATER_ONCE 5
//当日耗水总量(只上报)
//备注:
#define DPID_WATER_TOTAL 6
//电池电量(只上报)
//备注:
#define DPID_BATTERY_PERCENTAGE 7
//电池状态(只上报)
//备注:
#define DPID_BATTERY_STATE 8
//累计使用时间(只上报)
//备注:
#define DPID_USE_TIME 9
//天气延时(可下发可上报)
//备注:
#define DPID_WEATHER_DELAY 10
//灌溉时长(可下发可上报)
//备注:
#define DPID_COUNTDOWN 11
//工作状态(只上报)
//备注:
#define DPID_WORK_STATE 12
//智能天气(可下发可上报)
//备注:设备可以获取到的天气状态数据
#define DPID_SMART_WEATHER 13
//智能天气开关(可下发可上报)
//备注:
#define DPID_WEATHER_SWITCH 14
//单次使用时间(只上报)
//备注:
#define DPID_USE_TIME_ONE 15
//周期灌溉(可下发可上报)
//备注:自定义协议
#define DPID_CYCLE_TIMING 16
//普通定时(可下发可上报)
//备注:自定义协议
#define DPID_TIMER 17
//土壤湿度(只上报)
//备注:
#define DPID_SENSOR_HUMIDITY 18
//湿度上限设置(可下发可上报)
//备注:
#define DPID_MAXHUM_SET 20
//传感器设置(可下发可上报)
//备注:
#define DPID_SWITCH_ENABLED 25
...
void Irrigation_OnlyDeleteTimer(void);
