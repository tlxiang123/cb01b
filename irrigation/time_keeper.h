...
static volatile uint8_t s_need_sync = 1;   // 上电默认需要同步

void TimeKeeper_MarkNeedSync(void);   // 标记需要重新同步（不等于立即请求）
uint8_t TimeKeeper_NeedSync(void);    // 查询是否需要同步
void TimeKeeper_ClearNeedSync(void);  // 成功后清除
